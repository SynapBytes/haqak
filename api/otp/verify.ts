...
const PHONE_LIMIT_PER_WINDOW = 10;
...
export default async function handler(req: Req, res: Res): Promise<void> {
  ...
  if (!/^\d{6}$/.test(code)) {
    sendJson(res, 400, { error: "Invalid verification code format" });
    return;
  }
  ...
  try {
    ...
    if (result.status !== "approved") {
      sendJson(res, 401, { success: false, status: result.status });
      return;
    }
    ...
    sendJson(res, 502, { error: "Failed to verify OTP" });
  }
}
