...
const MPProfilePage = () => {
  ...
  const topCategories = stats ? Object.entries(stats.categories).sort((a, b) => b[1] - a[1]).slice(0, 5) : [];
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="container py-20 text-center">
          ...
          <h1 className="text-xl font-bold text-foreground mb-2">{t("mp_profile.not_found")}</h1>
          <p className="text-sm text-muted-foreground">{t("mp_profile.not_found_sub")}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="container py-8 px-4 max-w-4xl relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="civic-card mb-6">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5">
            <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl bg-gradient-to-br from-accent to-primary flex items-center justify-center shrink-0 shadow-lg">
              {profile.avatar_url ? (
                ...
                <span className="text-3xl sm:text-4xl font-bold text-white">{profile.full_name.charAt(0)}</span>
              )}
            </div>
            <div className="text-center sm:text-right flex-1">
              <div className="flex items-center justify-center sm:justify-start gap-2 mb-2">
                <h1 className="text-xl sm:text-2xl font-bold text-foreground">{profile.full_name}</h1>
                <Badge className="bg-accent/10 text-accent border-accent/20 text-xs">{t("mp_profile.badge")}</Badge>
              </div>
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 text-sm text-muted-foreground mt-3">
                {profile.governorate && (
                  <span className="flex items-center gap-1.5 bg-secondary px-3 py-1.5 rounded-lg"><Building2 className="w-3.5 h-3.5" />{profile.governorate}</span>
                )}
                {profile.constituency && (
                  <span className="flex items-center gap-1.5 bg-secondary px-3 py-1.5 rounded-lg"><MapPin className="w-3.5 h-3.5" />{profile.constituency}</span>
                )}
                {profile.center && (
                  <span className="flex items-center gap-1.5 bg-secondary px-3 py-1.5 rounded-lg"><Building2 className="w-3.5 h-3.5" />{profile.center}</span>
                )}
              </div>
              {stats && stats.total > 0 && (
                <div className="mt-4 max-w-xs mx-auto sm:mx-0">
                  <div className="flex justify-between items-center mb-1.5">
                    <span className="text-xs text-muted-foreground">{t("mp_profile.resolution_rate")}</span>
                    <span className="text-sm font-bold text-foreground">{resolutionRate}%</span>
                  </div>
                  <div className="w-full h-2.5 bg-secondary rounded-full overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${resolutionRate}%` }} transition={{ duration: 1, delay: 0.3 }} className="h-full rounded-full bg-gradient-to-l from-success to-success/70" />
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-4 mb-6">
          {[
            { label: t("mp_profile.resolved"), value: stats?.resolved || 0, icon: CheckCircle2, bgClass: "bg-success/10", textClass: "text-success", borderClass: "border-success/20" },
            { label: t("mp_profile.in_progress"), value: stats?.inProgress || 0, icon: Clock, bgClass: "bg-warning/10", textClass: "text-warning", borderClass: "border-warning/20" },
            { label: t("mp_profile.open"), value: stats?.received || 0, icon: AlertCircle, bgClass: "bg-destructive/10", textClass: "text-destructive", borderClass: "border-destructive/20" },
          ].map((stat, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.1 }} className={`rounded-2xl border ${stat.borderClass} ${stat.bgClass} p-4 md:p-5 text-center`}>
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-6">
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="civic-card">
            <h3 className="font-semibold text-foreground mb-4 flex items-center gap-2">
              <BarChart3 className="w-4 h-4 text-accent" />
              {t("mp_profile.performance")}
            </h3>
            <div className="space-y-3">
              {[
                { label: t("mp_profile.total_issues"), value: stats?.total || 0, icon: BarChart3 },
                { label: t("mp_profile.collective"), value: stats?.collective || 0, icon: Users },
                { label: t("mp_profile.citizen_confirmed"), value: stats?.confirmed || 0, icon: CheckCircle2 },
                { label: t("mp_profile.resolution_rate"), value: `${resolutionRate}%`, icon: TrendingUp },
              ].map((item, i) => (
                <div key={i} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                  <span className="flex items-center gap-2 text-sm text-muted-foreground"><item.icon className="w-3.5 h-3.5" />{item.label}</span>
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="civic-card">
            <h3 className="font-semibold text-foreground mb-4 flex items-center gap-2">
              <PieChart className="w-4 h-4 text-accent" />
              {t("mp_profile.top_categories")}
            </h3>
            {topCategories.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">{t("mp_profile.no_data")}</p>
            ) : (
              <div className="space-y-3">
                  ...
                  return (
                    <div key={cat}>
                      ...
                      <div className="w-full h-2 bg-secondary rounded-full overflow-hidden">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.8, delay: 0.7 + i * 0.1 }} className="h-full rounded-full bg-accent/70" />
                      </div>
                    </div>
                  );
                })}
